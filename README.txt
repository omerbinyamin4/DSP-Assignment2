# Assignment1DSP
    ...
    Output URL: s3://dsp2-hadoop/output4Sort/part-r-00000
...
# How to run :
    ...
    2. We create one Jar file that contains all classes, the only change between the steps is the main class to run.
       ...
       3. Run main using IntelliJ or other IDE.
       4. After the run, the output will be located at s3://dsp2-hadoop/Output4Submission/part-r-00000
...
# Map-Reduce Steps:
        ...
        1. TriGramMR:
            ...
            combiner:
                here we can use the combiner and reduce locally because our only data is number of occurrences
            reduce:
        ...
        2. ParamsMR
            ...
            combiner :
                we cant combine here. this will result with missing data because we want the reducer to sum it with all the values of the same key
            reduce:
        ...
        3. ProbCalc
            map: organizes the params to match one key (the trigram) in the reduce func
                (result of map for giver trigram t is:
                    ...
                    t, (nr10, *some value*))
            combiner:
                using combiner here is impossible too. as we've seen in class the sum of the division is not associative and will cause wrong calculation
            reduce:
        ...
        4. SortResults:
            ...
            combiner:
                no need. we want the reducer to sort all values per key.
            reduce: now when reducing all results by key (word1 word3, score) it is converted back to the form (word1 word2 word3, score)
